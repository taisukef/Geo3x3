...
// ------------------------------------------------------------
export const readTextFile = async (fn)=>{
  if (globalThis.Deno) {
...
// ----------------------
export function encodeCPU(lat, lng, level) {
  if (level < 1) return "";
  let res = "";
  let Lng = lng;
  if (Lng >= 0) {
...
export function decodeCPU(code) {
...
async function createComputePipelineCompat(device, module, entryPoint = "main") {
...
const ENCODER_WGSL = await readTextFile("./geo3x3_WGSL_encode.wgsl");
...
const DECODER_WGSL = await readTextFile("./geo3x3_WGSL_decode.wgsl");
...
`;
function u32ArrayFromCodes(codes, level) {
...
function stringifyFromDigits(first, digits, level) {
...
export async function encodeGPU(device, lats, lngs, level) {
  ...
  const inLatBuf = device.createBuffer({
    size: lats.byteLength,
    usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
  });
  const inLngBuf = device.createBuffer({
    size: lngs.byteLength,
    usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
  });
  device.queue.writeBuffer(inLatBuf, 0, lats.buffer);
  device.queue.writeBuffer(inLngBuf, 0, lngs.buffer);
  const outFirstBuf = device.createBuffer({
    size: count * 4,
    usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC
  });
  const outDigitsBuf = device.createBuffer({
    size: Math.max(1, digitsLen) * 4,
    usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC
  });
  ...
  const readDigits = device.createBuffer({
    size: Math.max(1, digitsLen) * 4,
    usage: GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST
  });
...
export async function decodeGPU(device, codes, level) {
  ...
  device.queue.writeBuffer(paramBuf, 0, params.buffer);
  const inFirstBuf = device.createBuffer({
    size: first.byteLength,
    usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
  });
  const inDigitsBuf = device.createBuffer({
    size: digits.byteLength || 4,
    usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
  });
  ...
  const outLatBuf = device.createBuffer({
    size: count * 4,
    usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC
  });
  const outLngBuf = device.createBuffer({
    size: count * 4,
    usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC
  });
  const outUnitBuf = device.createBuffer({
    size: count * 4,
    usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC
  });
  const module = device.createShaderModule({
  ...
  const readLat = device.createBuffer({
    size: count * 4,
    usage: GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST
  });
  const readLng = device.createBuffer({
    size: count * 4,
    usage: GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST
  });
  const readUnit = device.createBuffer({
    size: count * 4,
    usage: GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST
  });
